import { LitElement, html, PropertyValueMap } from 'lit';
import { property } from 'lit/decorators.js';
import { classMap } from 'lit/directives/class-map.js';
import "@polymer/iron-collapse/iron-collapse.js";
import "@polymer/iron-icon/iron-icon.js";
import "@polymer/iron-icons/hardware-icons.js";

export class OneupCollapseBase extends LitElement {

  @property({ type: String, reflect: true, attribute: 'title' })
  title = '';

  @property({ type: Boolean, reflect: true, attribute: 'opened' })
  opened = false;

  protected willUpdate(changedProperties: PropertyValueMap<OneupCollapseBase> | Map<PropertyKey, unknown>): void {
    if (changedProperties.has('opened')) {
      const detail = { value: this.opened };
      const event = new CustomEvent('opened-changed', { detail });
      this.dispatchEvent(event);
    }
  }

  protected toggle() {
    this.opened = !this.opened;
  }

  render() {
    return html`
      <div
        class="collapse-bar"
        @click=${() => this.toggle()}
      >
        <iron-icon
          class=${classMap({
            ['collapsed-' + this.opened]: true
          })}
          icon="hardware:keyboard-arrow-down"
        ></iron-icon>
        <div class="collapse-title">
          <i>${this.title}</i>
        </div>
      </div>
      <iron-collapse .opened=${this.opened}>
        <slot></slot>
      </iron-collapse>
    `;
  }
}
